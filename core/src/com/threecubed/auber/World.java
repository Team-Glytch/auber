...
public class World implements Saveable {
	...
	private List<Vector2> destroyedSystems = new ArrayList<Vector2>();
	...
	public final int[] backgroundLayersIds = { map.getLayers().getIndex("background_layer"), };

	/** IDs of layers that should be rendered infront of entities. */
	public final int[] foregroundLayersIds = { map.getLayers().getIndex("foreground_layer"),
	...
	public static enum Tiles {
		WALL_SYSTEM(38), STANDALONE_SYSTEM(62), STANDALONE_SYSTEM_LIGHT(50),

		WALL_SYSTEM_ATTACKED(40), STANDALONE_SYSTEM_ATTACKED(64), STANDALONE_SYSTEM_LIGHT_ATTACKED(52),

		WALL_SYSTEM_DESTROYED(42), STANDALONE_SYSTEM_DESTROYED(66), STANDALONE_SYSTEM_LIGHT_DESTROYED(54),
		 ...
		 * @throws IllegalArgumentException Thrown if no tile could be found with the
		 *                                  given ID
		 */
	...
	/**
	 * The shortest distance an NPC should move from its current position when
	 * fleeing.
	 */
	...
	public World(AuberGame game) {
		...
		for (int y = 0; y < navigationLayer.getHeight(); y++) {
			for (int x = 0; x < navigationLayer.getWidth(); x++) {
				...
				float[] cellCoordinates = { x * navigationLayer.getTileWidth(), y * navigationLayer.getTileHeight() };
				if (currentCell != null) {
	...
	public void updateSystemState(float x, float y, SystemStates newState) {
		...
		int[] systemPosition = { (int) x / collisionLayer.getTileWidth(),
		...
		if (targetTileId == World.Tiles.STANDALONE_SYSTEM.tileId
			...
			default:
				...
				// will think that theres a chance newSystem and newSystemLight may not have
				// been
			...
			TiledMapTileLayer foregroundLayer = (TiledMapTileLayer) map.getLayers().get("foreground_layer");
	...
	/**
	 * Return the state of a system, given the coordinates of the system object (not
	 * the tile).
	 *
	 * @param x The x coordinate to check
	 * @param y The y coordinate to check
	 * @return A {@link SystemStates} representing the state of the system
	 * @throws IllegalArgumentException if a standalone system light is at the
	...
	public SystemStates getSystemState(float x, float y) {
		...
		default:
			throw new IllegalArgumentException(
					"Use the coordinates of the System object on the".concat("tilemap - not the system tile."));
		}
	}
	...
	/**
	 * Check to see if any of the end conditions have been met, if so update the
	 * screen.
	 */
