...
/**
 * The GameEntity class is the abstract class from which all entities, including
 * the player must inherit from. It contains information regarding the speed and
 * max speed of entities, code to render an entity to the game world and for
 * entities like the {@link Player}, information information regarding friction
...
public abstract class GameEntity implements Saveable {
	...
	/**
	 * The "brain" of the entity, run any code that should be run at each render
	 * cycle that isn't related to rendering the entity.
	 *
	...
	/**
	 * Update the entity's position, taking into account any obstacles and their
	 * current velocity.
	 *
	...
	public void move(Vector2 velocity, TiledMap map) {
		...
		// Store the direction of the velocity. More efficient than calling function
		// multiple times,
		// also helps with the eventuality that the velocity sign might flip when
		// avoiding collisions
		...
		for (float[] offset : collisionOffsets) {
			...
			cell = collisionLayer.getCell((int) ((position.x + offset[0]) / collisionLayer.getTileWidth()),
	...
	/**
	 * Check if the {@link GameEntity} is currently visible on-screen. This will
	 * return true, even if the entity is only partially on the screen.
	 *
	 * @param world The game world
	 * @return A boolean dictating whether the entity is visible by the world's
	 *         camera
	 */
