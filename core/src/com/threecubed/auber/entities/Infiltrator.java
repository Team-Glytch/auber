 ...
 * system and sabotage them until caught by the {@link Player}.
 *
...
public class Infiltrator extends Npc {
	...
	public void handleDestinationReached(World world) {
		...
		if (oldState != States.FLEEING) {
			if (!playerNearby(world)
					&& Utils.randomFloatInRange(world.randomNumberGenerator, 0, 1) < World.SYSTEM_SABOTAGE_CHANCE) {
	...
	public void handleTeleporterShot(final World world) {
		...
		if (!exposed) {
			...
			position.x = Utils.randomFloatInRange(world.randomNumberGenerator, World.BRIG_BOUNDS[0][0],
					World.BRIG_BOUNDS[1][0]);
			position.y = Utils.randomFloatInRange(world.randomNumberGenerator, World.BRIG_BOUNDS[0][1],
					World.BRIG_BOUNDS[1][1]);
			aiEnabled = false;
		}
	}
