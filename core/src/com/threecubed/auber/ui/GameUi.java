...
public class GameUi {
	...
	private void drawChargeMeter(World world, SpriteBatch screenBatch) {
		screenBatch.begin();
		uiFont.draw(screenBatch, "Teleporter Ray Charge", CHARGE_METER_POSITION.x,
	...
	private void drawHealthbar(World world, SpriteBatch screenBatch) {
		screenBatch.begin();
		uiFont.draw(screenBatch, "Health", HEALTHBAR_POSITION.x, HEALTHBAR_POSITION.y + (HEALTHBAR_MAX_HEIGHT / 2));
		screenBatch.end();
		...
		shapeRenderer.begin(ShapeType.Filled);
		shapeRenderer.rect(HEALTHBAR_POSITION.x + 60f, HEALTHBAR_POSITION.y, HEALTHBAR_WIDTH, healthbarHeight);
		shapeRenderer.end();
	}
	...
	private void drawSystemWarnings(World world, SpriteBatch screenBatch) {
		...
		for (RectangleMapObject system : world.systems) {
			...
			arrowSprite.setPosition(SYSTEM_WARNINGS_POSITION.x - 20f, SYSTEM_WARNINGS_POSITION.y + offset - 10f);
			arrowSprite.setRotation(systemAngleVector.angleDeg() - 90f);
			arrowSprite.draw(screenBatch);

			String systemName = system.getName();
			// No need for DESTROYED case as when system destroyed, removed from systems
			// list
			...
			uiFont.draw(screenBatch, systemName, SYSTEM_WARNINGS_POSITION.x, SYSTEM_WARNINGS_POSITION.y + offset);
			offset += 25f;
